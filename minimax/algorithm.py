...
def evaluate_board(board):
    return num_pieces(board, 1) - num_pieces(board, 2) + (num_pieces(board, 3) - num_pieces(board, 4)) * 2
...
def move_piece(board, old_x, old_y, new_x, new_y):
    if (new_x, new_y) in get_valid_moves(old_x, old_y, board):
        board[new_x][new_y] = board[old_x][old_y]
        board[old_x][old_y] = 0
        king_piece(board, new_x, new_y)
...
def sim_move(x, y, move, board, capture):
  move_piece(board, x, y, move[0], move[1])
  if capture:
...
def get_all_moves(board, color):          #May be able to replace with get_valid_boards
  ...
  for x,y in get_piece_locations(board, color):
    ...
    for x1, y1, capture in valid_moves:     # May need to change this for doubles/triples
      temp_board = deepcopy(board)
      new_board = sim_move(x, y, (x1,y1), temp_board, capture)
